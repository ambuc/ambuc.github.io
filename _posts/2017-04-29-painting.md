...
`concat $ zipWith replicate node [1..]` takes a node like `[1,1,1,1]` and
returns a list like `[1,2,3,4]`. Taking `permutations` on that returns a list
like `[[1,2,3,4],[1,2,4,3]...]` etc. These are all real shufflings of the box
...
> pDist node = freqMap $ P.map decorate allNodes
>   where paint          :: [Int] -> [Int]
>         paint (x:y:xs) = x:x:xs
> 
>         decorate       :: [Int] -> [Int]
...
> 
>         allNodes      :: [[Int]]
>         allNodes      = permutations $ concat $ zipWith replicate node [1..]
>
> freqMap :: [[Int]] -> Map [Int] (Ratio Int)
> freqMap xs = Map.fromList
>            $ P.map ( second (% total) . (head &&& length) )
>            $ group $ sort xs
