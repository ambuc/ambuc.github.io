...
Sequences](http://oeis.org) to see if either `(1,1,6,6,141,141)`, `(1,6,141)`,
or  `(1,11,281)` appear at all. It turns out `(1,6,141)` are the first few
digits of [A241015](http://oeis.org/A241015), which is the
...
```python
def M(n):
  return (n-2)/2 if (n%2==0) else (n-1)/2
```
This will help us avoid having to calculate `f(n)` when `f(n-1)` is known.
